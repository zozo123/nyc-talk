...
Several sessions on this agenda sit close by, so name the difference in one sentence each. A bypass-path talk shows a correct control that is not on the route the agent takes; here the control is on the route, runs, and is correct, and what it accepts is agent-written. A talk on agents that had permission shows permission is not intent; here, for the answer key, the permission is the work itself, so the fix moves who owns the criteria rather than what is permitted. Talks on shared state files and CI pipelines each show one component trusting another's output; here that is the common shape of all four non-escape escapes in the abstract, recorded in one lab, each on the far side of a wall that held. If it sounds like the confused deputy, it is; the talk's claim is what agents change: far more deputies, and one where least privilege runs out. Sandboxes limit reach. Breaches happen at acceptance.
