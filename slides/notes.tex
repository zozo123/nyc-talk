...
{\itshape Speak to the supply-chain people. Every byte accounted for. Then: which bytes, not who had the right.}\par
...
{\itshape Why did the door stay open? Walk the picture. Two routes: what it reads, what it runs.}\par
...
{\itshape Say the message slowly. Then the show of hands, yours up first. Look around. Then: the permission is the work.}\par
...
{\itshape Row by row: the authority each acceptor now requires. Then back to the ticket: REJECT, the door never opens, ACCEPT.}\par
...
{\itshape The lesson for every security test. Four rows in order. Then our own mistake, plainly.}\par
